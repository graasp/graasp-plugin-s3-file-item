...
import { S3FileExtraContent, S3FileItemExtra } from '../interfaces/common';

export type GetMetadataFromItemTaskInputType = {
  item?: Item<S3FileItemExtra>;
};

class GetMetadataFromItemTask extends BaseTask<S3FileExtraContent> {
  private s3: S3;
  ...
  async run(handler: DatabaseTransactionHandler, log: FastifyLoggerInstance): Promise<void> {
    ...
    if ((size === 0 || size) && contenttype) {
      ...
      try {
        ...
        this._result = itemData.extra.s3File;
      } catch (error) {
